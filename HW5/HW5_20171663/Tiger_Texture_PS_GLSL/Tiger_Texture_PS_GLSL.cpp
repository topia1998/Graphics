...
GLint loc_blind_effect, loc_blind_timer;
...
glm::mat4 Ben_ModelViewMatrix;
glm::mat3 Ben_ModelViewMatrixInvTrans;
glm::mat4 ModelMatrix_Ben_to_eye;
...
int mode_draw_screen, flag_screen_effect, flag_draw_screen;
int key;
int mcamera;
...
void initialize_Camera(char key) {
	...
	case '0':
		...
		ViewMatrix = glm::lookAt(glm::vec3(500.0f, 500.0f, 500.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		cur = 0;
		break;
	case '1':
		camera.zoom_factor = 1.0f;
...
void set_material_ironman() {
	glUniform4fv(loc_material_GS.ambient_color, 1, material_ironman.ambient_color);
	glUniform4fv(loc_material_GS.diffuse_color, 1, material_ironman.diffuse_color);
	glUniform4fv(loc_material_GS.specular_color, 1, material_ironman.specular_color);
	glUniform1f(loc_material_GS.specular_exponent, material_ironman.specular_exponent);
	glUniform4fv(loc_material_GS.emissive_color, 1, material_ironman.emissive_color);
}
...
void display(void) {
	...
	ben_route = abs((ben_clock - 720) / 360) * 180;
	...
	ModelViewProjectionMatrix = ProjectionMatrix * ModelViewMatrix;
	Ben_ModelViewMatrix = ModelViewMatrix;
	ModelViewMatrixInvTrans = glm::inverseTranspose(glm::mat3(ModelViewMatrix));
	Ben_ModelViewMatrixInvTrans = ModelViewMatrixInvTrans;
	...
	glm::vec4 position_EC = Ben_ModelViewMatrix * glm::vec4(light[3].position[0], light[3].position[1],
		light[3].position[2], light[3].position[3]);
	glUniform4fv(loc_light[3].position, 1, &position_EC[0]);
	glm::vec3 direction_EC = glm::mat3(Ben_ModelViewMatrixInvTrans) * glm::vec3(light[3].spot_direction[0],
...
void timer_scene(int value) {
	...
	light_timer = (timestamp_scene % 360);
	glutPostRedisplay();
...
void keyboard(unsigned char key, int x, int y) {
	...
	if (mcamera == 0) {
		if (key == 'w') {
			glUseProgram(h_ShaderProgram_TXPS);
			if (light[0].light_on == 0) light[0].light_on = 1;
			else if (light[0].light_on == 1) light[0].light_on = 0;
			if (light[1].light_on == 0) light[1].light_on = 1;
			else if (light[1].light_on == 1) light[1].light_on = 0;
		...
		if (key == 'm') {
			glUseProgram(h_ShaderProgram_TXPS);
			if (light[3].light_on == 0) light[3].light_on = 1;
			else if (light[3].light_on == 1) light[3].light_on = 0;
	...
	if (mcamera == 1) {
		if (key == 'e') {
			glUseProgram(h_ShaderProgram_TXPS);
			if (light[2].light_on == 0) light[2].light_on = 1;
			else if (light[2].light_on == 1) light[2].light_on = 0;
...
void reshape(int width, int height) {
	...
	ProjectionMatrix = glm::perspective(camera.zoom_factor * 45.0f * TO_RADIAN, camera.aspect_ratio, 100.0f, 20000.0f);
	ViewProjectionMatrix = ProjectionMatrix * ViewMatrix;
	glutPostRedisplay();
}
...
void initialize_OpenGL(void) {
	...
	//	glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	ViewMatrix = glm::lookAt(glm::vec3(500.0f, 500.0f, 500.0f),
		glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	camera.zoom_factor = 1.0f;
...
void set_up_scene_lights(void) {
	...
	light[2].light_on = 0;
	light[2].position[0] = eye_x; light[2].position[1] = eye_y; // spot light position in EC
	light[2].position[2] = eye_z; light[2].position[3] = 1.0f;
	...
	light[3].ambient_color[0] = 0.2125f; light[3].ambient_color[1] = 0.1275f;
	light[3].ambient_color[2] = 0.054f; light[3].ambient_color[3] = 1.0f;

	light[3].diffuse_color[0] = 0.714f; light[3].diffuse_color[1] = 0.4284f;
	light[3].diffuse_color[2] = 0.181f; light[3].diffuse_color[3] = 1.0f;

	light[3].specular_color[0] = 0.3935f; light[3].specular_color[1] = 0.271f;
	light[3].specular_color[2] = 0.166f; light[3].specular_color[3] = 1.0f;

	light[3].spot_direction[0] = 1.0f; light[3].spot_direction[1] = -1.0f; 
	light[3].spot_direction[2] = -5.0f;
	...
	//light 3
	glUniform1i(loc_light[3].light_on, light[3].light_on);
	position_EC = Ben_ModelViewMatrix * glm::vec4(light[3].position[0], light[3].position[1], light[3].position[2], light[3].position[3]);
	glUniform4fv(loc_light[3].position, 1, &position_EC[0]);
	...
	direction_EC = glm::mat3(Ben_ModelViewMatrix) * glm::vec3(light[3].spot_direction[0], light[3].spot_direction[1], light[3].spot_direction[2]);
	glUniform3fv(loc_light[3].spot_direction, 1, &direction_EC[0]);
